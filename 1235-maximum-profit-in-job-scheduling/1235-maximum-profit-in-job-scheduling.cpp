...
public:
    ...
    int findMaxProfit(vector<vector<int>>& jobs, vector<int>& startTime, int n, int pos) {
        ...
        // Binary search: return iterator to first ele which is >= than ele at jobs[pos][1]
        auto nextItr = lower_bound(startTime.begin(), startTime.end(), jobs[pos][1]) - startTime.begin();
        
        // Take it or leave it
        int maxProfit = max(findMaxProfit(jobs, startTime, n, pos+1), jobs[pos][2] + findMaxProfit(jobs, startTime, n, nextItr));
